...
public class DropZoneL2M3 : MonoBehaviour
{
    private GameObject placedPuzzlePiece;

    [Header("DropZones yang akan dijumlahkan")]
    public DropZoneL2M3[] linkedDropZones; // Diatur dari Inspector
    ...
    public int GetPlacedPuzzleID()
    {
        ...
        {
            DragAndDropL2M3 script = placedPuzzlePiece.GetComponent<DragAndDropL2M3>();
            if (script != null)
    ...
    bool AllDropZonesOccupied()
    {
        ...
        foreach (DropZoneL2M3 zone in linkedDropZones)
        {
    ...
    void CheckSum()
    {
        ...
        DadangController controller = FindObjectOfType<DadangController>();
        if (controller != null)
        {
        ...
        {
            Debug.LogWarning("⚠️ Controller (DadangController) tidak ditemukan di scene!");
        }
    }
