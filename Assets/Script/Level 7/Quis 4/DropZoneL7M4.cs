...
public class DropZoneL7M4 : MonoBehaviour
{
    ...
    public DropZoneL7M4[] allDropZonesToCheck;

    private static bool hasChecked = false; // agar hanya dicek sekali
    public NpcControllerL7M4 controller;
    ...
    public int GetPlacedID()
    {
        ...
        {
            var drag = placedPuzzlePiece.GetComponent<DragAndDropL7M4>();
            if (drag != null)
    ...
    {
        foreach (DropZoneL7M4 dz in allDropZonesToCheck)
        {
            if (!dz.IsOccupied())
                return false;
        }
    ...
    public void ManualCheckAllDropZones()
    {
        ...
        NpcControllerL7M4 controller = FindObjectOfType<NpcControllerL7M4>();
        if (controller != null)
        {
