...
public class DropZoneL5M5 : MonoBehaviour
{
    ...
    public DropZoneL5M5[] allDropZonesToCheck;

    private static bool hasChecked = false; // agar hanya dicek sekali
    public NpcControllerL5M5 controller;
    ...
    public int GetPlacedID()
    {
        ...
        {
            var drag = placedPuzzlePiece.GetComponent<DragAndDropL5M5>();
            if (drag != null)
    ...
    private void CheckAllDropZonesIfReady()
    {
        ...
        foreach (DropZoneL5M5 dz in allDropZonesToCheck)
        {
            if (!dz.IsOccupied())
        ...
        NpcControllerL5M5 controller = FindObjectOfType<NpcControllerL5M5>();
        if (controller != null)
        {
